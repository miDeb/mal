...
pub fn init_env(env: &mut Env) {
    ...
    env.set(
        "list".to_string(),
        Value::Fn(Rc::new(|list| Ok(Value::List(list.to_vec())))),
    );
    env.set(
        "list?".to_string(),
        Value::Fn(Rc::new(|list| {
            Ok(Value::Bool(matches!(list[0], Value::List(_))))
        })),
    );
    env.set(
        "empty?".to_string(),
    ...
    env.set(
        "=".to_string(),
        Value::Fn(Rc::new(|list| Ok(Value::Bool(list[0] == list[1])))),
    );
    env.set(
        "<".to_string(),
        Value::Fn(Rc::new(|list| Ok(Value::Bool(list[0] < list[1])))),
